(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', '.media-upload', function(event) {
			...
			var isColumnBg = $(this).data('column-bg');
            ...
            semplice_file_frame.on('select', function() {
				...
				if(type === 'gallery') {
					...
					if(type === 'background') {
						...
						} else if(isColumnBg === 'columnbg') {
							$(columnId).find('input[name=background-image]').first().val(attachment.url);
							$(columnId).find('img.column-bg-image-preview').first().attr('src', attachment.url);
							$(columnId).find('img.column-bg-image-preview').first().show();
						} else {
